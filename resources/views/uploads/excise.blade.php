<x-layouts.app title="Excise upload">
    ...
    <section class="card upload-card">
        ...
        <div class="upload-body"><form method="post" enctype="multipart/form-data">@csrf<label class="drop-zone"><input type="file" name="indent" accept="application/pdf,.pdf" required><span>Select Excise PDF</span><small data-file-name>Proforma Indent PDF</small></label><button class="btn" type="submit" style="margin-top:16px">Upload and preview</button></form><p class="note">Nothing changes stock at this stage. The indent identity is read and checked.</p></div>
    </section>
    @if($preview)
    <section class="card" style="margin-top:16px">
        ...
        <div class="table-wrap"><table><thead><tr><th>Document product</th><th>Common brand</th><th>Code</th><th>Received stock</th><th>Total ml</th><th>Bottle price</th><th>Line amount</th></tr></thead><tbody>
        @foreach($preview['lines'] as $item)
            <tr class="{{ !empty($item['common_product_id']) ? 'mapping-ok' : 'mapping-needs-attention' }}"><td>{{ $item['name'] }}</td><td>@include('components.upload-mapping', ['source' => 'excise'])</td><td>{{ $item['code'] }}</td><td>{{ \App\Support\StockFormatter::bottlesAndMl($item['volume_ml'], $item['size_ml'], true) }}</td><td>{{ number_format($item['volume_ml']) }}</td><td>₹{{ number_format($item['bottle_price'], 2) }}</td><td>₹{{ number_format($item['amount'], 2) }}</td></tr>
        @endforeach
        </tbody></table></div>
    </section>
